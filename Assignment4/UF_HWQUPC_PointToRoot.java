 ...
 */
public class UF_HWQUPC_PointToRoot implements UF {
    /**
     ...
     */
    public UF_HWQUPC_PointToRoot(int n, boolean pathCompression) {
        count = n;
        parent = new int[n];
        height = new int[n];
        for (int i = 0; i < n; i++) {
     ...
     * @throws IllegalArgumentException if {@code n < 0}
     */
    public UF_HWQUPC_PointToRoot(int n) {
        this(n, true);
    }
    ...
    private void mergeComponents(int i, int j) {
    	...
    	if  (height[p] < height[q]) { parent[p] = q; height[q] += height[p]; } 
    	else { parent[q] = p; height[p] += height[q]; }  
    }

    /**
     * This implements the single-pass path-halving mechanism of path compression
     */
    private void doPathCompression(int root, int p) {
