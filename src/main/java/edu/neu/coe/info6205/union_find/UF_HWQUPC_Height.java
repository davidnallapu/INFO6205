 ...
 */
public class UF_HWQUPC_Height implements UF {
    /**
     ...
     */
    public UF_HWQUPC_Height(int n, boolean pathCompression) {
        count = n;
        parent = new int[n];
        height = new int[n];
        for (int i = 0; i < n; i++) {
    ...
    private void mergeComponents(int i, int j) {
    	...
    	if      (height[p] < height[q]) parent[p] = q;
        else if (height[p] > height[q]) parent[q] = p;
        else {
            parent[q] = p;
            height[p]++;
        }  
    ...
    /**
     * This implements the single-passpath-halving mechanism of path compression
     */
